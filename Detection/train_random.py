...
class Trainer(object):
    ...
    def train(self):
        ...
        OUTDIR = 'seed{}_{}_random_ckpt'.format(self.args.seed, self.args.imp_type)
        OUTDIRS = OUTDIR + '/model{}'.format(self.args.imp_num)
        if not os.path.exists(OUTDIRS): os.makedirs(OUTDIRS)
        
        print('-' * 100)
        print("Begining RANDOM Pruning, Sparsity:[{}]".format(self.args.imp_num))
        print('-' * 100)

        pruning.pruning_model(self.yolov4._Build_Model__yolov4.backbone, (1 - (0.8 ** self.args.imp_num)), exclude_first=True, random=True)
        ...
        pruning.see_zero_rate(self.yolov4._Build_Model__yolov4.backbone)
        save_name = './' + OUTDIR + '/backbone_random{}.pth'.format(self.args.imp_num + 1)
