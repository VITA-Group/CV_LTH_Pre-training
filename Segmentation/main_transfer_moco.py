...
def main():
    ...
    moco_ckpt = torch.load("./moco_v2_800ep_pretrain.pth.tar", map_location="cuda")['state_dict']
    
    if opts.mask_dir == "":

        print("Transfer MOCO 100% Model")
        model_state_dict = model.module.backbone.state_dict()
        update_state_dict = {k[17:] : v for k , v in moco_ckpt.items() if k[17:] in model_state_dict.keys()} 
        model_state_dict.update(update_state_dict)
        model.module.backbone.load_state_dict(model_state_dict)
        print("Load MOCO model:[{}/{}]".format(len(update_state_dict), len(model_state_dict)))

    else:
        print("Transfer Mask dir:[{}]".format(opts.mask_dir))
        mask_dict = torch.load(opts.mask_dir, map_location="cuda")
        # mask_dict = {k[17:] : v for k, v in mask_ckpt.items() if "module.encoder_q" in k and "mask" in k}

        pruning.moco_pruning_model_custom_res50v1(model.module.backbone, mask_dict, no_conv1=True)
        pruning.see_zero_rate(model.module.backbone)
        model_state_dict = model.module.backbone.state_dict()
        
        unpruned_state_dict = {k[17:] : v for k , v in moco_ckpt.items() if k[17:] in model_state_dict.keys()} 
        pruned_state_dict = {k[17:] + '_orig': v for k , v in moco_ckpt.items() if k[17:] + '_orig' in model_state_dict.keys()}

        model_state_dict.update(unpruned_state_dict)
        model_state_dict.update(pruned_state_dict)

        model.module.backbone.load_state_dict(model_state_dict)
        pruning.see_zero_rate(model.module.backbone)

        ori_len = len(model_state_dict)
        overlap_len = len(unpruned_state_dict) + len(pruned_state_dict)

        print("Load: OverLap/Ori: [{}/{}]".format(overlap_len, ori_len))
        print("Change MOCO Mask Dir:[{}]".format(opts.mask_dir))
