...
def main():
    ...
    if opts.mask_dir == "":
        ...
        print("Transfer Simclr 100% Model")
        ...
        mask_ckpt = torch.load(opts.mask_dir, map_location="cuda")
        mask_dict = pruning.extract_mask(mask_ckpt)    # 80% mask dict
        pruning.simclr_pruning_model_custom_res50v1(model.module.backbone, mask_dict)
        pruning.see_zero_rate(model.module.backbone)
        model_state_dict = model.module.backbone.state_dict()
        
        
        unpruned_state_dict = {k : v for k , v in sim_ckpt.items() if k in model_state_dict.keys()} 
        pruned_state_dict = {k + '_orig': v for k , v in sim_ckpt.items() if k + '_orig' in model_state_dict.keys()}
        ...
        print("Change SimCLR Mask Dir:[{}]".format(opts.mask_dir))
